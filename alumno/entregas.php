      ...
      <div class="app-title">
        ...
        <?php if ($rowa > 0) {
            while($data = $querya->fetch()){
                ...
                $sqln= "SELECT * FROM notas as n INNER JOIN ev_entregadas as ev ON n.ev_entregada_id = ev.ev_entregada_id
                INNER JOIN alumnos as a ON ev.alumno_id = a.alumno_id WHERE n.ev_entregada_id = $ev_entregada AND a.alumno_id = $idAlumno";
                 ...
                 if ($datan > 0) {
                    ...
                    $valor = '<kbd class="bg-success">Sin Calificar</kbd>';
                    $calificacion = '';
                 }
        ?>        
    ...
    <?php if($fecha < $fechaLimite){ ?>
        ...
        <?php }else{ ?>
            <div class="row bg-danger p-3 text-white">
                <h5>Ya no se pueden hacer entregas (Fecha Limite <?= $fechaLimite; ?></h5>
            </div>
        <?php } ?>
        <?php } ?>
      <div class="row">
