...
?>
   ...
   <div class="row">
    <div class="col-md-12 border shadow p-2 bg-info text-white">
      <h3 class="display-4 text-center" >SISTEMA ACADEMICO LUIS ESPINAL</h3>
    </div>
   </div>
   ...
   <div class="row">
    <?php if($row > 0) {
      while($data = $query->fetch()){
        ...
        <div class="card m-1 shadow" style="width: 19rem;">
            <img src="images/materia.jpg" class="card-img-top" alt="...">
            <div class="card-body">
              ...
              <kbd class="bg-info"><?= $data['nombre_aula']?> </kbd></h5>
              <a href="contenido.php?curso=<?= $data['pm_id']?>" class="btn btn-primary">Acceder</a>
              <a href="alumnos.php?curso=<?= $data['pm_id']?>" class="btn btn-primary">Ver alumnos/notas</a>
              <a href="asistencia.php?curso=<?= $data['pm_id']?>" class="btn btn-primary">Asistencia</a>
              <a href="citaciones.php?curso=<?= $data['pm_id']?>" class="btn btn-primary">Citaciones</a>
            </div>           
        </div>
        </div> 
    <?php  } }
